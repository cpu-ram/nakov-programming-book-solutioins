...
namespace Matrices
{
    public class Matrix
    {
        ...
        public Matrix(string sourceFile)
        {
            ...
            using (streamReader)
            {
                ...
                int dimensionsInt = Convert.ToInt32(dimensionsString);
                yDim = xDim = dimensionsInt;
                elements = new int[dimensionsInt, dimensionsInt];

                for (int i = 0; i < dimensionsInt; i++)
                {
                    string tempString = streamReader.ReadLine();
                    char separator = Convert.ToChar(" ");
                    string[] tempStringArr = tempString.Split(separator);
                    for (int j = 0; j < tempStringArr.Length; j++)
                    {
                        elements[i, j] = Convert.ToInt32(tempStringArr[j]);
                    }
                }
            }
        ...
        {
            StreamWriter fileSaver = new StreamWriter(fileLocation);
            string tempString = "";
            using (fileSaver)
            {
                for(int i=0; i < elements.GetLength(0); i++)
                {
                    ...
                    fileSaver.WriteLine(tempString);
                }
            }
        }
    }
    ...
    class Program
    {
        static void Test()
        {
            ...
            Matrix greatestSumPlatform = Matrix.FindGreatestPlatform(matrix, 2);

            string platformFile = "platform.txt";
            greatestSumPlatform.Export(platformFile);
            TextEditor.ReadFile(platformFile);
