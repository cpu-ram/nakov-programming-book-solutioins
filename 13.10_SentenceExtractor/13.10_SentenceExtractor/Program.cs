...
namespace StringEditor
{
    public class StringSearcher
    {
        ...
        {
            Symbol, Letter, Word, Sentence, Paragraph
        }
        ...
        public static void Tokenize(string entryString, UnitType unitType, bool printResults = false)
        {
            int[] symbolCounter = new int[65536];
            StringBuilder sb = new StringBuilder();
            string pattern = ".";
            MatchCollection matches = Regex.Matches(entryString, pattern);
            for (int i = 0; i < matches.Count; i++)
            {
                string tempString = Convert.ToString(matches[i]);
                char tempChar = Convert.ToChar(tempString);
                int charIntCode = Convert.ToInt32(tempChar);
                symbolCounter[charIntCode] += 1;
            }
            for (int i = 0; i < symbolCounter.Length; i++)
            {
                if (symbolCounter[i] > 0)
                {
                    char tempCharTwo = Convert.ToChar(i);
                    Console.WriteLine("{0}: {1} occurences", tempCharTwo, symbolCounter[i]);
                }
            }
        }
        public static void TokenizeWords(string entryString, UnitType unitType, bool printResults = false)
        {
            string tempString = entryString;
            tempString = tempString.ToLower();
            SortedDictionary<string, int> dictionary = new SortedDictionary<string, int>();
            string pattern = "[a-zA-Z]+";
            MatchCollection matches = Regex.Matches(tempString, pattern);

            for(int i=0; i<matches.Count; i++)
            {
                AddWord(dictionary, Convert.ToString(matches[i]));
            }
            Console.Write("");

            static void AddWord(SortedDictionary<string, int> dictionary, string entryString)
            {
                
                if (dictionary.ContainsKey(entryString))
                {
                    dictionary[entryString] += 1;
                }
                else
                {
                    dictionary.Add(entryString, 1);
                }
    ...
    {
        public string EnterString(string stringName="string")
        {
    ...
    class Program
    {
        ...
        {
            string testString = "Sometimes I live in country and sometimes I live in town.";
            StringSearcher.TokenizeWords(testString, StringSearcher.UnitType.Word);
